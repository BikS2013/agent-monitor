/**
 * Build script to generate and export sample data with different sizes
 * Supports small, medium, and large dataset generation
 */
import fs from 'fs';
...
if (!['small', 'medium', 'large'].includes(size)) {
  console.error('Invalid size specified. Use "small", "medium", or "large"');
  process.exit(1);
}

console.log(`Starting ${size} dataset generation...`);
...
const sizeConfig = {
  small: {
    recentConversations: 10,  // Last 24 hours
    weekConversations: 20,    // Last week (excluding last 24 hours)
    monthConversations: 30,   // Last month (excluding last week)
    olderConversations: 40,   // Older than a month
    messageMultiplier: 0.8,   // Slightly fewer messages per conversation
    collectionsCount: 5,      
    groupsCount: 3
  },
  medium: {
    recentConversations: 50,  // Last 24 hours
    weekConversations: 100,   // Last week (excluding last 24 hours)
    monthConversations: 150,  // Last month (excluding last week)
    olderConversations: 200,  // Older than a month
    messageMultiplier: 1,     // Standard number of messages
    collectionsCount: 10,
    groupsCount: 5
  },
  large: {
    recentConversations: 200, // Last 24 hours
    weekConversations: 300,   // Last week (excluding last 24 hours)
    monthConversations: 500,  // Last month (excluding last week)
    olderConversations: 1000, // Older than a month
    messageMultiplier: 1.2,   // More messages per conversation
    collectionsCount: 18,
    groupsCount: 8
  }
};
...
const generateConversation = (
  ...
  // Adjust message count based on size configuration
  const messageCount = Math.max(3, Math.floor(baseMessageCount * config.messageMultiplier));
...
const generateSampleData = () => {
  console.log(`Generating ${size} sample data...`);
  ...
  console.log(`Generating conversations and messages for ${size} dataset...`);
  
  let totalMessages = 0;
  
  // Generate recent conversations (last 24 hours)
  for (let i = 0; i < config.recentConversations; i++) {
    const id = `c${i + 1}`;
  ...
  for (let i = 0; i < config.weekConversations; i++) {
    const id = `c${config.recentConversations + i + 1}`;
    const userId = userData[Math.floor(Math.random() * userData.length)].id;
    const aiAgentId = aiAgentData[Math.floor(Math.random() * aiAgentData.length)].id;
  ...
  for (let i = 0; i < config.monthConversations; i++) {
    const id = `c${config.recentConversations + config.weekConversations + i + 1}`;
    const userId = userData[Math.floor(Math.random() * userData.length)].id;
    const aiAgentId = aiAgentData[Math.floor(Math.random() * aiAgentData.length)].id;
  ...
  for (let i = 0; i < config.olderConversations; i++) {
    const id = `c${config.recentConversations + config.weekConversations + config.monthConversations + i + 1}`;
    const userId = userData[Math.floor(Math.random() * userData.length)].id;
    const aiAgentId = aiAgentData[Math.floor(Math.random() * aiAgentData.length)].id;
  ...
  // Use a subset of collection templates based on the configuration
  const collectionsToGenerate = Math.min(config.collectionsCount, collectionTemplates.length);
  
  for (let i = 0; i < collectionsToGenerate; i++) {
    const id = `col${i + 1}`;
    ...
    // Each collection gets between 10% and 20% of all conversations
    const minSize = Math.max(5, Math.floor(conversationIds.length * 0.1));
    const maxSize = Math.min(conversationIds.length, Math.floor(conversationIds.length * 0.2));
    const collectionSize = Math.floor(Math.random() * (maxSize - minSize)) + minSize;
  ...
  // Use a subset of group templates based on the configuration
  const groupsToGenerate = Math.min(config.groupsCount, groupTemplates.length);
  ...
  for (let i = 0; i < groupsToGenerate; i++) {
    ...
    // Each group gets between 2 and 5 collections (or fewer if there aren't enough collections)
    const groupSize = Math.min(collectionIds.length, Math.floor(Math.random() * 3) + 2);
    const groupCollections = collectionIds
      .sort(() => 0.5 - Math.random())
      .slice(0, groupSize);
    
    // Select 2-3 admin users
    const shuffledUsers = [...userData.map(u => u.id)].sort(() => 0.5 - Math.random());
    const adminCount = Math.min(shuffledUsers.length, Math.floor(Math.random() * 2) + 2);
    ...
    const viewerCount = Math.min(otherUsers.length, Math.floor(Math.random() * 2) + 1);
...
console.log('Generated data statistics:', {
  messagesCount: Object.keys(sampleData.messages).length,
  conversationsCount: Object.keys(sampleData.conversations).length,
  collectionsCount: Object.keys(sampleData.collections).length,
  groupsCount: Object.keys(sampleData.groups).length,
  aiAgentsCount: Object.keys(sampleData.aiAgents).length,
  usersCount: Object.keys(sampleData.users).length
});
...
// Output file path with size in filename
const outputFile = path.join(outputDir, `${size}SampleData.json`);
